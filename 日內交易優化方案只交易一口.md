...
| A4 | 校準集 | 5 | 5 | +310 | +2200 | 1932 | 401 | 1.11 | 6.79 | 172 | 3.57 | 保留 |
| A5 | 校準集 | 5 | 5 | +310 | +2200 | 1932 | 401 | 1.11 | 6.79 | 172 | 3.57 | 待觀察 |

校準集補充觀察：

- baseline：`trades=5`、`pnl=+310`、`max dd=1932`、`PF=1.11`
- `A1 ~ A5` 都沒有出現「幾乎無交易」問題，但 `rejects / entry_checks` 比例都高於 `0.80`
- 第二輪重判後，`A4` 被列為正式候選，因為它在不減少交易筆數的前提下，同時改善總損益、最大回撤與 PF
- `A5` 與 `A4` 結果相同，但參數更保守且沒有額外收益，因此暫列 `待觀察`，不優先進驗證集
...
2. A4 與 A5 的回測結果完全相同，但 A4 參數較不保守，表示 A5 的額外限制沒有帶來額外收益。
3. 因此優先保留 A4 作為正式候選；A5 保留為備選參考，不列入本輪主驗證名單。
```
...
> 說明：這裡同時標註「文件完成度」與「實作完成度」。

| 章節 | 文件完成度 | 實作完成度 | 現況摘要 |
|:---|:---:|:---:|:---|
| 1. Implementation Plan | 已完成 | 已完成 | 目標、範圍、不做什麼、風險、驗證方式都已定義 |
| 2. Module Architecture | 已完成 | 部分完成 | 架構圖與模組邊界已定，但一口模式核心模組仍在收斂中 |
| 3. 一口模式參數規劃 | 已完成 | 部分完成 | 參數表已建立，但仍需依驗證集/保留集持續微調 |
| 4. Task List | 已完成 | 部分完成 | T1~T7 已拆好，部分任務已實作，仍有收尾項目 |
| 5. 驗收標準 | 已完成 | 部分完成 | DoD 已定義，但最終收斂尚未全部達成 |
...
- 如果只看文件，1-6 都已完成。
- 如果看實作，2、3、4、5 還在持續收斂，1 與 6 已完成。
