...
flowchart TD
  subgraph UI["Dashboard / API Layer"]
    API["dashboard/app.py (REST)"]
    WS["dashboard/websocket.py (WS 推播)"]
    FE["dashboard/static/* (前端)"]
  end

  subgraph Engine["Core Engine Layer"]
    TE["core/engine/__init__.py TradingEngine (Facade)"]
    EV["core/engine/events.py EventProcessor (事件/決策路由)"]
    EX["core/engine/executor.py OrderExecutor (下單/平倉)"]
    HM["core/engine/health.py HealthMonitor (心跳/核對/異常)"]
    Q["core/engine/queries.py EngineQueries (查詢聚合)"]
  end

  subgraph Domain["Domain Layers"]
    STRAT["strategy/* (Strategy: 產生 Signal)"]
    RISK["risk/* (Risk: 放行/拒絕/熔斷)"]
    POS["core/position.py PositionManager (持倉/交易紀錄)"]
    MD["core/market_data.py (Aggregator/Indicators/Snapshot)"]
